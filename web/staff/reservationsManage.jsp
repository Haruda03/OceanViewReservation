<%-- 
    Document   : manageReservations
    Created on : Feb 25, 2026, 10:10:44 PM
    Author     : Haruda
--%>
...
<head>
  ...
  <style>
    ...
    .btn-sm{ padding:8px 10px; border-radius:10px; border:none; background:#00c3ff; color:#fff; font-weight:900; cursor:pointer; }
    .btn-danger{ padding:8px 10px; border-radius:10px; border:none; background:#e64b4b; color:#fff; font-weight:900; cursor:pointer; }
    .row-actions{ display:flex; gap:8px; flex-wrap:wrap; }
    select{ padding:8px 10px; border-radius:10px; border:1px solid #cfd9e6; }
...
<div class="container">
  <div class="hero">
    ...
    <p>Filter and manage bookings/reservations. Staff can cancel or mark a confirmed stay as completed.</p>
  </div>
  ...
  <div class="card" style="margin-top:16px;">
    <h3>Filter</h3>
    <form method="get" action="<%=request.getContextPath()%>/staff/reservations-manage" style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
    ...
    <table>
      ...
      %>
        <tr>
          ...
          <td><span class="chip"><%= r.getRequestType() %></span></td>
          <td>
            <%= r.getRoomType() %>
          ...
          <td><%= nights %></td>
          <td><%= r.getGuests() %></td>
          <td><span class="chip <%= r.getStatus() %>"><%= r.getStatus() %></span></td>
          <td><%= (r.getExpiresAt()==null ? "-" : r.getExpiresAt()) %></td>
          <td>
            <div class="row-actions">
              <% if("CONFIRMED".equals(r.getStatus())) { %>
                <form method="post" action="<%=request.getContextPath()%>/staff/reservations-manage">
